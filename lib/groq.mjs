...
// Used when the main model keeps returning something unparseable mid-call.
...
export async function chatJSON(messages, { model = TURN_MODEL, attempts = 4 } = {}) {
  let lastError;

  // Retry on the same model twice, then switch. A model that returns empty content
  // once tends to do it again on the same input, and on a live call a wrong-sounding
  // answer from the smaller model beats "I'm having trouble, goodbye".
  const chain = Array.from({ length: attempts }, (_, i) =>
    i < Math.ceil(attempts / 2) ? model : FALLBACK_MODEL
  );
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      lastError = err;
      console.warn(`groq ${attemptModel} attempt ${i + 1}/${attempts} failed: ${err.message}`);
